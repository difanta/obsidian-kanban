...
export class HeadlessStateManager {
  ...
  init(data: string, shouldParseData: boolean) {
    if (shouldParseData) {
  ...
  async saveToDisk() {
    ...
    await window.app.vault.modify(this.file, fileStr);
  }
  ...
  async reparseBoardFromMd() {
    try {
      this.setState(
        await this.getParsedBoard(await window.app.vault.read(this.file)),
        false
      );
    } catch (e) {
      console.error(e);
